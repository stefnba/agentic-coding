...
## Boundary with `decision`

A naming or term choice is a glossary entry, not a decision record. A term choice that
encodes a contested architectural trade-off routes to the `decision` skill instead.

## Reporting back

Show the lines that changed and nothing more — imitate `backlog`'s reporting shape. Don't
print the whole glossary and don't summarise its state.
