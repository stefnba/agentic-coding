...
Not every choice earns a record. `${CLAUDE_PLUGIN_ROOT}/workflow/artifacts.md` owns the bar and
where a choice that misses it belongs instead; check the decision against it before writing, and say
plainly when it doesn't clear it. Declining to write the file is the right outcome more often than
not.
...
3. **Tag `areas:` by reusing a term already in use.** The records you just read carry the
   ...
   and `${CLAUDE_PLUGIN_ROOT}/workflow/artifacts.md` owns why. Coin a new area only when nothing
   there fits, never as a near-synonym of a term that does.
