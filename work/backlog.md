...
- [docs] decision 0015 carries one stale ownership sentence ("workflow doc describes both strategies generically") after the in-flight ID-6 amendment
  - argument mostly unaffected (per-repo git.md and setup choice stand; decision 0016 flipped the trunk recommendation) — catch in a reconciliation sweep or supersede, precedent: 0013's handling of 0002/0012
- [skills] setup's strategy question and implement/ship's mode-conditional branching are unverified — substantially rewritten for decision 0016 (bundle-first names, worktrees, cleanup), still text-verified only, never run
  - per writing-for-agents references/testing.md: does setup ask/skip the question correctly on first-run vs re-run and never overwrite an existing valid git.md; does implement create `<bundle-id>/bundle` and `<bundle-id>/ticket/<NN-slug>` in worktrees per the declared mode, sync the bundle branch, and sweep merged leftovers; does ship land `<bundle-id>/bundle` and clean up trees and branches — this repo's own docs/agents/git.md (bundle-branch) is the one real instance to test against
