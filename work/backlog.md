...
- [skills] shape lands the bundle by direct push to the default branch — a protected default branch rejects that; shape's PR path is undefined
- [skills] setup's strategy question and the bundle-git skill are unverified as prompts — git mechanics smoke-tested in a scratch repo; agent-following-the-skill runs still missing
  - per writing-for-agents references/testing.md: setup's ask/skip and no-overwrite, bundle-git open/close/check including the halt cases, implement/ship actually delegating — this repo's own git.md (bundle-branch) is the real instance
