...
# 04 — Workflow doc, git conventions rule, one-home sweep
...
surface, not a file to create here. Last ticket, so it also runs the one-home sweep (AC-2, the two enumeration
strings) across everything 01–03 wrote. Ordered after 02/03 so the sweep and the doc describe the
finished state.
...
Skill or agent edits (→ 02, 03). No edits to decision 0015 and no restated strategy
